...
var ewdGateway = {
  ...
  addToEWDQueue: function(query, headersString, contentEsc, request, response) {
    ...
    if (ewdGateway.trace) console.log("added to Queue (http): " + queuedRequest.query + "; queue length = " + qLength + "; requestNo = " + this.totalRequests + "; after " + this.elapsedTime() + " sec");
  ...
  addToSocketQueue: function(message,client) {
    if (ewdGateway.trace) console.log("addToSocketQueue - message = " + message);
    var messageObj = JSON.parse(message);
    var messageType = messageObj.type;
    if (ewdGateway.trace) console.log("messageType = " + messageType);
    var token = messageObj.token;
    if (ewdGateway.trace) console.log("token = " + token);
    ...
    if (messageType === 'register') {
      ...
      if (ewdGateway.trace) console.log("client " + client.id + " registered for token " + token);
      return;
    }
  ...
  addToGlobalAccessQueue: function(queuedRequest) {
    ...
    var qLength = this.requestQueue.length;
    if (qLength > this.maxQueueLength) this.maxQueueLength = qLength;
    if (ewdGateway.trace) console.log("added to Queue (globalAccess): " + queuedRequest.method);
    // trigger the processing of the queue
    this.queueEvent.emit("processEWDQueue");
  },
  ...
  dbOutput:function(connection) {
    ...
    this.db[connection].stdout.on('data', function (data) {
      ...
      if (ewdGateway.trace) {
        if (ewdGateway.trace) console.log("from " + ewdGateway.databaseName + " on connection " + connection + ":" + "\r\n" + dataStr + "\r\n=================\r\n");
        var dump = 'dump: ';
      ...
      if (contentStr.substr(-len) === terminator) {
        ...
        if (ewdGateway.db[connection].type === 'globalAccess') {
          ...
          if (ewdGateway.trace) console.log("globalAccess - returning results for " + method);
          ...
          */
          pieces = contentStr.split(ewdGateway.eol + "\x14\x13\x12\x11");
          if( pieces.length > 1 ) {
            /* 
          ...
          if (method === 'mFunction') {
            ...
            ewdGateway.db[connection].callback(error,{value:value,output:outputStr});
          }
        ...
        if (ewdGateway.db[connection].type === 'socket') {
          if (ewdGateway.trace) console.log("socket - sending response back to browser");
          ...
          for (var messageNo = 0; messageNo < messages.length; messageNo++) {
            ...
            if (ewdGateway.trace) console.log("*** message=" + message);
            try {
            ...
            switch (messageType) {
              ...
              case 'noSession':
                ...
                if (ewdGateway.trace) console.log("NoSession - token = " + token);
              ...
              case 'serverSend':
                if (ewdGateway.trace) console.log("serverSend: contentStr=" + message);
                type = messageObj.subType;
                token = messageObj.token;
                if (ewdGateway.trace) console.log("serverSend: token = " + token);
                if (typeof ewdGateway.socketClientByToken[token] !== 'undefined') {
                  var socketClient = ewdGateway.socketClientByToken[token];
                  if (ewdGateway.socketClient[socketClient.id].connected) {
                    socketClient.json.send({type: type, message: message});
                  }
                }
                break;
              ...
              default:
                if (ewdGateway.trace) console.log("messageType: " + messageType + "; message=" + message);
                if (typeof ewdGateway.db[connection] !== 'undefined') {
                  if (typeof ewdGateway.db[connection].client !== 'undefined') {
        ...
        if (!preBlockResponse) {
          ...
          ewdGateway.ewdMap.module[req.app][ewdGateway.ewdMap.method[req.app][req.page]](sessArr[1], function(error, results) {
            ...
            if (ewdGateway.trace) console.log("sessid = " + sessArr[1]);
  ...
  processEWDQueue: function() {
    if (!ewdGateway.processingEWDQueue) {
      ...
      if (ewdGateway.trace) console.log("processing queue: " + ewdGateway.queueEvents + "; queue length " + ewdGateway.requestQueue.length + "; after " + ewdGateway.elapsedTime() + " seconds");
      var queuedRequest;
      ...
      if (ewdGateway.requestQueue.length > 0) {
        if (ewdGateway.trace) console.log("queue processing abandoned as no free proceses available");
      }
      ...
      switch (type) {
        ...
        case 'socket':
          ...
          if (typeof ewdGateway.messageHandler[queuedRequest.messageType] !== 'undefined') {
            if (ewdGateway.trace) console.log("invoking custom method " + queuedRequest.messageType);
            ewdGateway.db[connection].isAvailable = true;
            ...
            if (ewdGateway.trace) console.log("ewdGetFragment: " + queuedRequest.targetId);
            this.db[connection].client = queuedRequest.client;
            this.db[connection].stdin.write(type + "\r\n" + queuedRequest.messageType + "\r\n" + queuedRequest.token + "\r\n" + queuedRequest.page + "\r\n" + queuedRequest.targetId + "\r\n" + queuedRequest.nvp + "\r\n");
          }

          else {
            if (ewdGateway.trace) console.log("sending message");
            this.db[connection].client = queuedRequest.client;
  ...
  invokeGlobalAccessCommand: function(request, connection) {
    if (ewdGateway.trace) console.log("invokeGlobalAccessCommand: " + request.method + ": connection " + connection);
    ...
    */
    ...
    if (startWebserver) {
      ...
      ewdGateway.io.sockets.on('connection', function(client){
        if (ewdGateway.trace) console.log("socket connected: " + client.id);
        if (ewdGateway.socketClient[client.id]) {
          ewdGateway.socketClient[client.id].connected = true;
          if (ewdGateway.trace) console.log("socketClient connection set back to true");
        }
        else {
          if (ewdGateway.trace) console.log("that client.id hasn't been recognised");
        }
        client.connected = true;

        client.on('message', function(message){
          if (ewdGateway.trace) console.log("From browser: " + message);
          //for (i=0;i<message.length;i++) dump = dump + ":" + message.charCodeAt(i);
          //console.log(dump);
          ewdGateway.addToSocketQueue(message,client);
        });
        client.on('disconnect', function() {
          if (ewdGateway.trace) console.log("socket disconnected: " + client.id);
